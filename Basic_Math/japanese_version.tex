...
\begin{titlepage}
  ...
  \vspace{0.4cm}
  {\large デジタル版: \url{https://www.andreyis.com}\par}
  \vspace{0.8cm}
  {\large \today\par}
\end{titlepage}
...
\textbf{証明}\ \\
\noindent
任意に $M > 0$ を与える。$N = \lfloor M \rfloor + 1$ とおくと、すべての $n \geq N$ に対して
\[
n \geq N = \lfloor M \rfloor + 1 > M
\]
となる。$M > 0$ は任意であったから、$\lim_{n \to \infty} n = +\infty$。 \qed
\end{example}
\kaisetsu{$\lfloor M \rfloor$ は $M$ を超えない最大の整数です。例えば $\lfloor 3.7 \rfloor = 3$、$\lfloor 5 \rfloor = 5$ です。}
\end{examplebox}
...
\textbf{証明}\ \\
\noindent
任意に $M < 0$ を与える。不等式 $5 - n < M$ は $n > 5 - M$ と同値である。$M < 0$ より $5 - M > 5$ であるから、$N = \lfloor 5 - M \rfloor + 1$ とおけば、すべての $n \geq N$ に対して
\[
n \geq N > 5 - M \implies 5 - n < M
\]
となる。$M < 0$ は任意であったから、$\lim_{n \to \infty} (5 - n) = -\infty$。 \qed
\end{example}
\end{examplebox}
...
\textbf{証明}\ \\
\noindent
任意に $M > 0$ を与える。$13n + 1 > M$ は $n > \frac{M-1}{13}$ と同値である。$N = \max\left\{1,\left\lfloor\frac{M-1}{13}\right\rfloor + 1\right\}$ とおくと、すべての $n \geq N$ に対して
\[
13n + 1 \geq 13N + 1 > 13 \cdot \frac{M-1}{13} + 1 = M
\]
となる。$M > 0$ は任意であったから、$\lim_{n \to \infty} (13n + 1) = +\infty$。 \qed
\end{example}
\end{examplebox}
...
\item $\lim_{n \to \infty} a_n = L$ より、正数 $\frac{\eps}{2(|M|+1)}$ に対してある $N_2 \in \N$ が存在し、$n \geq N_2$ で $|a_n - L| < \frac{\eps}{2(|M|+1)}$。
\end{itemize}
...
$+\infty$ への発散 & $\forall M > 0,\ \exists N \in \N$ s.t. $n \geq N \implies a_n > M$ \\
$-\infty$ への発散 & $\forall M < 0,\ \exists N \in \N$ s.t. $n \geq N \implies a_n < M$ \\
